...
import {
  GetAllCommentsForCurrentUserCommand
} from "../../public/comments/use-cases/get-all-comments-for-current-user.use-case";
...
export class BloggersController {
  ...
  async getAllCommentsForAllPosts(
    ...
    const posts = await Promise.all(blogs.map(blog => this.commandBus.execute(
      new GetAllPostsForCurrentUserCommand(blog.id, currentUser.id))
    ));
    const comments = await Promise.all(posts.flat().map(post => this.commandBus.execute(
      new GetAllCommentsForCurrentUserCommand(post.id, currentUser.id)
    )));
